...
const [,out='shot.png',hover]=process.argv
if(hover){ const c=page.locator('[data-jv-interactive]',{hasText:new RegExp(hover,'i')}).first(); await c.hover({force:true}); await page.waitForTimeout(500) }
await page.screenshot({path:process.env.OUT||out}); await browser.close(); server.close(); console.log('shot',process.env.OUT||out)
